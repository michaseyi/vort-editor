...
["_stbi_load","_stbi_load_from_memory","_stbi_image_free","_memory","___indirect_function_table","___asyncjs__load_string_emscripten","__print_running_environment","__get_canvas_size","__update_canvas_size","_main","onRuntimeInitialized"].forEach((prop) => {
...
function __asyncjs__load_string_emscripten(path) { return Asyncify.handleAsync(async () => { try { const dataPath = UTF8ToString(path); const data = new Uint8Array(await fetch(dataPath).then( function(res) { return res.arrayBuffer(); })); const length = data.length; const ptr = _malloc(length + 1); writeArrayToMemory(data, ptr); HEAP8.set(new Uint8Array([0]), ptr + length); return ptr; } catch { return 0; } }); }
function _print_running_environment() { console.log(typeof importScripts == 'undefined' ? "Web" : "Worker"); }
function _get_canvas_size(width,height,selector) { const canvas = document.querySelector(UTF8ToString(selector)); const canvas_width = new Int32Array([canvas._width]); const canvas_height = new Int32Array([canvas._height]); HEAP32.set(canvas_width, width / 4 | 0); HEAP32.set(canvas_height, height / 4 | 0); }
function _update_canvas_size(selector) { const canvas = document.querySelector(UTF8ToString(selector)); canvas.width = canvas._width | 0; canvas.height = canvas._height | 0; canvas.style.width = `${(canvas.width / window.devicePixelRatio) | 0} px`; canvas.style.height = `${(canvas.height / window.devicePixelRatio) | 0} px`; }
...
var wasmImports = {
  ...
  /** @export */
  __asyncjs__load_string_emscripten,
  /** @export */
