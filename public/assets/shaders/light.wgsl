...
fn compute_point_light(light: PointLight, position: vec3<f32>, normal: vec3<f32>) -> vec3<f32> {
    let light_vector = normalize(light.position - position);
    let illumination_factor: f32 = max(0.0, dot(light_vector, normal));
    var attenuation_factor = compute_attenuation_factor(light.attenuation_coefficients, distance(light.position, position));
    let view_dir = normalize(common_uniforms.camera.position - position);
...
fn compute_directional_light(light: DirectionalLight, normal: vec3<f32>) -> vec3<f32> {
    let illumination_factor: f32 = max(0.0, dot(-light.direction, normal));
